...
project_dir = os.path.join(file_dir, os.path.pardir)
print("project dir: ", project_dir)
lib_path = os.path.join(project_dir, "libdarknet.so")
print(lib_path)
...
if __name__ == "__main__":
    ...
    meta = load_meta(os.path.join(project_dir, "cfg/coco.data").encode())
    input_file = os.path.join(project_dir, "data/dog.jpg").encode()
    output_file = os.path.join(project_dir, "data/dog_detections.txt")
    detections = detect(net, meta, input_file)
    print(detections)
    frame = 1
    track_id = -1
    visibility = 1
    ...
    with open(output_file, "w", newline="") as out:
        for detection in detections:
            ...
            class_id = mot_classes.get(class_pred.decode(), mot_classes['other'])
