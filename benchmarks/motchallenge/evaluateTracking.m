...
fprintf("seqmap: " + seqmap + "\n")
fprintf("resDir: " + resDir + "\n")
fprintf("gtDataDir: " + gtDataDir + "\n")
...
for ind = 1:length(allSequences)
    ...
    if ~multicam
        ...
        if ~exist(resFilename,'file')
            error('Invalid submission. Result (%s) for sequence %s not available!\n', resFilename, sequenceName);
        end
